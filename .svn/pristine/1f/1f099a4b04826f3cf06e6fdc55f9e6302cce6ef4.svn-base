...
$(document).ready(function() {
    ...
    $('input[name="salaryOption"], #inputSalary, #nonTaxable, input[name="severance"], #family, #children').on('input change', calculateSalary);
    $('.plus, .minus').on('click', calculateSalary);
    
    function calculateSalary() {
        const salaryOption = $('input[name="salaryOption"]:checked').val();  //급여기준
        const inputSalary = parseFloat($('#inputSalary').val().replace(/,/g, '')) || 0;  // 연봉 혹은 월급액
        ...
        let monthlySalary;
     	// 급여 기준에 따른 월급 계산
        if (salaryOption === "1") { // 연봉 선택
            // 퇴직금 포함 여부에 따른 월급 계산
            if (severance === 'Y') {
                monthlySalary = inputSalary / 13; 
                // 퇴직금 포함
            } else {
            ...
            monthlySalary = inputSalary; // 월급 자체로 사용
            $('#option_a3, #option_a4').prop('disabled', true); // 퇴직금 비활성화
        }
//         const monthlySalary = (salaryOption === "1") ? inputSalary / 12 : inputSalary; //연봉을 월급으로 전환

        let taxableIncome = monthlySalary - nonTaxable; //월급에서 비과세액 제외
        
        // 각종 사회보험료 계산
       	const nationalPension = Math.min(Math.max(16650, taxableIncome * 0.045), 265500); // 국민연금
        const healthInsurance = Math.min(Math.max(9890, taxableIncome * 0.03545),3911280); //건강보험
        const longTermCare = Math.max(1280, healthInsurance * 0.1295); //장기요양
        const employmentInsurance = Math.max(0, taxableIncome * 0.009); //고용보험

        // 과세표준 계산
        const monthlyTaxableIncome = monthlySalary - nonTaxable - nationalPension - healthInsurance - longTermCare - employmentInsurance;
        const yearlyTaxableIncome = monthlyTaxableIncome * 12;

        // 세액 계산
        const yearlyTax = calculateTax(yearlyTaxableIncome, family, children, severance);
        let monthlyTax = Math.floor(yearlyTax / 12);
	
        // 지방소득세
        let localIncomeTax = Math.floor(monthlyTax * 0.1);

        // 총 공제액 계산
        let totalDeductions = Math.floor(nationalPension + healthInsurance + longTermCare + employmentInsurance + monthlyTax + localIncomeTax);

     	// 실수령액 계산
        let netSalary = Math.max(0, Math.floor(monthlySalary - totalDeductions)); // 마이너스일 경우 0으로
        
        if (netSalary <= 1000000) {
            monthlyTax = 0;
            localIncomeTax = 0;
            totalDeductions = Math.floor(nationalPension + healthInsurance + longTermCare + employmentInsurance);
            netSalary = Math.max(0, Math.floor(monthlySalary - totalDeductions));
        }
        ...
        $('#incomeTax').text(monthlyTax.toLocaleString());
        $('#localIncomeTax').text(localIncomeTax.toLocaleString());
        $('#totalDeductions').text(totalDeductions.toLocaleString());
        $('#netSalary').text(netSalary.toLocaleString());
    };
		...
		<div class="card2">
			<div class="card2-body">
				...
				<div id="salaryCalPage">
					        ...
					        <div class="btnDiv">
					            <div class="btn-group btn-group-toggle custom-btn-group" data-toggle="buttons">
					                <label class="btn btn-secondary">
					                    <input type="radio" name="salaryOption" id="option_a1" autocomplete="off" checked="" value="1"> 연봉
					                </label>
					                <label class="btn btn-secondary">
					                    <input type="radio" name="salaryOption" id="option_a2" autocomplete="off" value="2"> 월급
					                </label>
					            </div>
					        </div>
					        <div class="btnDiv">
					...
					<div id="result">
						<div id="topDiv">
						<p id="topText">월 예상 실수령액</p>
						<p><span id="netSalary">0</span> 원</p>
						</div>
						<div id="resultBox">
						<p>한 달 기준 공제액</p>
