...
<div class="submainAll">
	...
	/outsou/WebList?outordMlsfc=OULC01-001&currentPage=1&srvcLevelCd=&srvcTeamscaleCd=&ord=rdcnt
	 -->
	<form id="searchForm" method="GET" action="/outsou/WebList">
		 <input type="hidden" name="outordMlsfc" value="${param.outordMlsfc}" />
	    <input type="hidden" name="currentPage" value="${param.currentPage}" />
	    <input type="hidden" id="ord" name="ord" value="${param.ord}" />
	    	
		    <c:choose>
		        <c:when test="${param.outordMlsfc == 'OULC01-001'}">
		        	   <p>프로그램 제작</p>
		        </c:when>
		        <c:when test="${param.outordMlsfc == 'OULC01-002'}">
		          	 <p>쇼핑몰 제작</p>
		        </c:when>
		        <c:when test="${param.outordMlsfc == 'OULC01-003'}">
		            <p>랜딩 페이지</p>
		        </c:when>
		        <c:otherwise>
		            <p>웹 제작</p>
				...
				<ul class="pagination">
					...
					<c:if test="${articlePage.currentPage gt 1}">
					    <li class="paginate_button page-item first">
					        <a href="javascript:void(0);" data-href="/outsou/WebList?outordMlsfc=${param.outordMlsfc}&currentPage=1"
					...
					<c:forEach var="pNo" begin="${articlePage.startPage}" end="${articlePage.endPage}">
					    <li class="paginate_button page-item ${pNo == articlePage.currentPage ? 'active' : ''}">
					        <a href="javascript:void(0);" data-href="/outsou/WebList?outordMlsfc=${param.outordMlsfc}&currentPage=${pNo}"
					...
					<c:if test="${articlePage.endPage lt articlePage.totalPages}">
					    <li class="paginate_button page-item next">
					        <a href="javascript:void(0);" data-href="/outsou/WebList?outordMlsfc=${param.outordMlsfc}&currentPage=${articlePage.endPage + 1}"
					           class="page-link dynamic-filter-link">&gt;</a>
					    </li>
					</c:if>
					...
					<c:if test="${articlePage.currentPage lt articlePage.totalPages}">
					    <li class="paginate_button page-item last">
					        <a href="javascript:void(0);" data-href="/outsou/WebList?outordMlsfc=${param.outordMlsfc}&currentPage=${articlePage.totalPages}"
					           class="page-link dynamic-filter-link">&gt;&gt;</a>
					    </li>
					</c:if>
