...
<div class="submainAll">
	...
	<form id="searchForm" method="GET" action="/outsou/PGList">
	    ...
	    <input type="hidden" id="ord" name="ord" value="${param.ord}" />
		    ...
		    <c:choose>
		        <c:when test="${param.outordMlsfc == 'OULC01-004'}">
		        	   <p>크롤링·스크래핑</p>
		        </c:when>
		        <c:when test="${param.outordMlsfc == 'OULC01-005'}">
		          	 <p>서버·클라우드</p>
		        </c:when>
		        <c:when test="${param.outordMlsfc == 'OULC01-006'}">
		            <p>수익자동화</p>
		        </c:when>
		        <c:otherwise>
		            <p>프로그램</p>
				...
				<ul class="pagination">
					...
					<c:if test="${articlePage.currentPage gt 1}">
					    <li class="paginate_button page-item first">
					        <a href="javascript:void(0);" data-href="/outsou/PGList?outordMlsfc=${param.outordMlsfc}&currentPage=1"
					...
					<c:forEach var="pNo" begin="${articlePage.startPage}" end="${articlePage.endPage}">
					    <li class="paginate_button page-item ${pNo == articlePage.currentPage ? 'active' : ''}">
					        <a href="javascript:void(0);" data-href="/outsou/PGList?outordMlsfc=${param.outordMlsfc}&currentPage=${pNo}"
					...
					<c:if test="${articlePage.endPage lt articlePage.totalPages}">
					    <li class="paginate_button page-item next">
					        <a href="javascript:void(0);" data-href="/outsou/PGList?outordMlsfc=${param.outordMlsfc}&currentPage=${articlePage.endPage + 1}"
					           class="page-link dynamic-filter-link">&gt;</a>
					    </li>
					</c:if>
					...
					<c:if test="${articlePage.currentPage lt articlePage.totalPages}">
					    <li class="paginate_button page-item last">
					        <a href="javascript:void(0);" data-href="/outsou/PGList?outordMlsfc=${param.outordMlsfc}&currentPage=${articlePage.totalPages}"
					           class="page-link dynamic-filter-link">&gt;&gt;</a>
					    </li>
					</c:if>
